# Ici je définis une fonction permettant de vérifier que l'utilisateur rentre correctement le genre de son nourrisson.
def inputGender():
    while True:
...
# Ici je définis une fonction permettant de vérifier que l'utilisateur rentre un age entre 0 et 60 mois. Si l'entrée n'est pas bonne je la redemande.
def inputAge():
    while True:
        try:
            age = int(input("Veuillez entrer l'age de votre nourrisson en mois (entre 0 et 60 mois): "))
        except ValueError:
            print("La valeur que vous avez entrée n'est pas valide. Rentrez un entier svp.")
            continue

        if age >= 0 and age <= 60:
            return age
        else:
            print("Le nombre que vous avez rentré n'est pas valide, il faut qu'il soit compris en 0 et 60.")
            continue

# Ici je définis une fonction permettant de vérifier que l'utilisateur rentre un nombre à virgule. Si l'entrée n'est pas bonne je la redemande.
def inputFloat(message):
    while True:
        try:
            userInput = float(input(message))
        except ValueError:
            print("La valeur que vous avez entrée n'est pas valide. Rentrez un nombre à virgule svp (avec un point comme séparateur décimal).")
            continue
...
gender = inputGender()
age = inputAge()
weight = inputFloat("Veuillez entrer le poids de votre nourrisson en kg : ")
height = inputFloat("Veuillez entrer la taille de votre nourrisson en cm : ")
skull = inputFloat("Veuillez entrer la périmètre cranien de votre nourrisson en cm : ")

# Vérifications des constantes
if gender == "g":
    # Informations sur le poids
    print(f'\nLa norme de poids pour un garçon de {age} mois est situé entre {low_weights_boys[age]} kg et {high_weights_boys[age]} kg')
    if weight > low_weights_boys[age] and weight < high_weights_boys[age]:
        print(f'Le poids de votre nourrisson ({weight} kg) est dans la norme !')
    else:
        print(f'Le poids de votre nourrisson ({weight} kg) est hors norme !')
    
    # Informations sur la taille
    print(f'\nLa norme de taille pour un garçon de {age} mois est situé entre {low_heights_boys[age]} cm et {high_heights_boys[age]} cm')
    if height > low_heights_boys[age] and height < high_heights_boys[age]:
        print(f'La taille de votre nourrisson ({height} cm) est dans la norme !')
    else:
        print(f'La taille de votre nourrisson ({height} cm) est hors norme !')
    
    # Informations sur le périmètre cranien
    print(f'\nLa norme de périmètre cranien pour un garçon de {age} mois est situé entre {low_skulls_boys[age]} cm et {high_skulls_boys[age]} cm')
    if skull > low_skulls_boys[age] and skull < high_skulls_boys[age]:
        print(f'Le périmètre cranien de votre nourrisson ({skull} cm) est dans la norme !')
    else:
        print(f'Le périmètre cranien de votre nourrisson ({skull} cm) est hors norme !')

elif gender == "f":
    # Informations sur le poids
    print(f'\nLa norme de poids pour une fille de {age} mois est situé entre {low_weights_girls[age]} kg et {high_weights_girls[age]} kg')
    if weight > low_weights_girls[age] and weight < high_weights_girls[age]:
        print(f'Le poids de votre nourrisson ({weight} kg) est dans la norme !')
    else:
        print(f'Le poids de votre nourrisson ({weight} kg) est hors norme !')

    # Informations sur la taille
    print(f'\nLa norme de taille pour une fille de {age} mois est situé entre {low_heights_girls[age]} cm et {high_heights_girls[age]} cm')
    if height > low_heights_girls[age] and height < high_heights_girls[age]:
        print(f'La taille de votre nourrisson ({height} cm) est dans la norme !')
    else:
        print(f'La taille de votre nourrisson ({height} cm) est hors norme !')
    
    # Informations sur le périmètre cranien
    print(f'\nLa norme de périmètre cranien pour une fille de {age} mois est situé entre {low_skulls_girls[age]} cm et {high_skulls_girls[age]} cm')
    if skull > low_skulls_girls[age] and skull < high_skulls_girls[age]:
        print(f'Le périmètre cranien de votre nourrisson ({skull} cm) est dans la norme !')
    else:
        print(f'Le périmètre cranien de votre nourrisson ({skull} cm) est hors norme !')